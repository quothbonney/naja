...
namespace naja {
    namespace gpu {
        ...
        struct CUBLASHandle {
            ...
            /// @brief Perform y = alpha * A(op) * x + beta * y with transpose option
            template <typename Real>
            __host__ void GeMV(const DMatrix<Real>& A, const DVector<Real>& x, DVector<Real>& y, Real alpha, Real beta, cublasOperation_t transa) {
                int m = A.rows;
                int n = A.cols;
                int lda = A.rows;
                // If transa == CUBLAS_OP_T, dimensions are swapped for GEMV
                int rows = (transa == CUBLAS_OP_N) ? m : n;
                int cols = (transa == CUBLAS_OP_N) ? n : m;
                (void)rows; (void)cols; // avoid unused warnings
                CUBLAS_CHECK(cublasDgemv(h, transa, m, n, &alpha, A.dmat, lda, x.dvec, 1, &beta, y.dvec, 1));
            }
            ...
            /// @brief Perform x = alpha * x for vectors
            template <typename Real>
            __host__ void Scal(DVector<Real>& x, Real alpha) {
                CUBLAS_CHECK(cublasDscal(h, x.len, &alpha, x.dvec, 1));
            }

            /// @brief y = y + alpha * x (AXPY)
            template <typename Real>
            __host__ void Axpy(const DVector<Real>& x, DVector<Real>& y, Real alpha) {
                CUBLAS_CHECK(cublasDaxpy(h, y.len, &alpha, x.dvec, 1, y.dvec, 1));
            }

            /// @brief dot = x^T y
            template <typename Real>
            __host__ void Dot(const DVector<Real>& x, const DVector<Real>& y, Real& result) {
                CUBLAS_CHECK(cublasDdot(h, x.len, x.dvec, 1, y.dvec, 1, &result));
            }
