...
std::pair<double, double> jacobi_rotation_cs(double var_i, double var_j, double cov_ij) {
    ...
    // If cov is exactly zero, theta=0 is a valid diagonalizing rotation.
    if (cov_ij == 0.0) {
        return {1.0, 0.0};
    }

    // For S = [var_i cov; cov var_j], the Jacobi angle that zeros the off-diagonal of R^T S R
    // (with R = [c -s; s c]) satisfies: tan(2θ) = 2*cov / (var_i - var_j).
