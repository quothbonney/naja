...
[[noreturn]] void die_usage(const std::string& msg) {
    ...
    std::cerr << "  naja sample bulk --models-root <dir> --model-list <file> --out-root <dir> --name <runname> --gpus <csv> --n-chains <n> --n-samples <n> [--tpb <n>] [--backmap] [--write-npy] [--bounds-policy ignore|filter] [--bounds-eps <eps>] [--write-samples-valid] [--verbose] [--quiet] [--dry-run] [--print-config] [--skip-existing] [--resume-from <run_dir>]\n";
